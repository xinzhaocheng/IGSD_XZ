...

{
  ...
  # ============================ jiaxun style Erope LNG ==============================================
  
  p_lng_europe_jiaxun <- ggplot(
    plot_df_lng_grouped2_eu,   # 👉需修改：用与上图一致过滤后的数据
    aes(x = Source, y = value, fill = group)
  ) +
    geom_col(position = "stack", width = 0.75) +
    facet_wrap(~ year, nrow = 1) +
    
    # 👉需修改：用 pal_legend_lng 而不是 pal_group_full
    # 👉需修改：breaks 用 present_lng_eu，而不是 legend_breaks
    scale_fill_manual(
      values = pal_legend_lng,
      breaks = present_lng_eu,
      name = "LNG Source"
    ) +
    
    labs(x = NULL, y = "EJ", title = "Traded LNG – Europe") +
    ...
    theme(
      ...
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank(),
      plot.margin        = margin(t = 18, r = 35, b = 12, l = 8, unit = "pt")
    )
  
  # 保存
  ggsave(
    filename = file.path(output_dir, "testya_LNG_Europe_sources_jiaxun_style.png"),
    plot = p_lng_europe_jiaxun,
    width = 8.0, height = 4.6, bg = "white", dpi = 300
  )
  
  # =========================== B. PIPE（Europe 汇总，drop 欧洲来源；首次出现顺序着色） ===========================
  plot_df_pipe_grouped_eu <- df_all %>%
    dplyr::filter(
      Variable == "traded gas by tech",
      trimws(region) %in% europe_parts,
      stringr::str_detect(technology, stringr::regex("pipeline", ignore_case = TRUE))
    ) %>%
    dplyr::mutate(src_base = extract_region_from_pipe(technology)) %>%
    dplyr::filter(!(src_base %in% europe_parts)) %>%
    dplyr::select(Source, technology, src_base, dplyr::all_of(yrs_use)) %>%
    tidyr::pivot_longer(dplyr::all_of(yrs_use), names_to = "year", values_to = "value") %>%
    dplyr::mutate(
    ...
    dplyr::group_by(Source, year, group0) %>%
    dplyr::summarise(value = sum(value, na.rm = TRUE), .groups = "drop") %>%
    dplyr::mutate(group = factor(group0))
  
  plot_df_pipe_grouped2_eu <- plot_df_pipe_grouped_eu %>%
    dplyr::filter(year %in% yrs_keep) %>%
    dplyr::filter((year == "2021" & Source == scen_levels[1]) | year != "2021")
  
  pipe_pos <- add_x_positions(plot_df_pipe_grouped2_eu, yrs_keep, scen_levels, eps, centers)
  
  present_pipe_eu <- pipe_pos$df2 %>%
    dplyr::arrange(match(year, yrs_keep), x) %>%
    dplyr::distinct(group) %>% dplyr::pull(group) %>% as.character()
  
  pal_pipe_final <- auto_palette_for_pipe(present_pipe_eu)
  
  p_pipe_eu <- ggplot2::ggplot(pipe_pos$df2, ggplot2::aes(x = x, y = value, fill = group)) +
    ggplot2::geom_col(width = 0.05) +
    ggplot2::scale_fill_manual(values = pal_pipe_final, breaks = present_pipe_eu,
                               limits = present_pipe_eu, drop = FALSE, name = "Pipeline Source") +
    ggplot2::scale_x_continuous(
      breaks = pipe_pos$x_breaks, labels = pipe_pos$x_labels,
      expand = ggplot2::expansion(mult = c(0.01, 0.10))
    ) +
    ggplot2::labs(x = NULL, y = "Natural Gas (EJ)") +
    ggplot2::theme_bw(base_size = 10) +
  ...
  p_lng_europe_jiaxun <- ggplot(plot_df_lng_grouped,
                         aes(x = Source, y = value, fill = group)) +
    geom_col(position = "stack", width = 0.75) +
    facet_wrap(~ year, nrow = 1) +
    scale_fill_manual(values = pal_group_full, breaks = legend_breaks, name = "LNG Source") +
    labs(x = NULL, y = "EJ", title = "Traded LNG – Europe") +
    theme_bw(base_size = 11) +
    theme(
      legend.position    = "right",
  ...
  ggsave(
    filename = file.path(output_dir, "testya_LNG_Europe_sources_fixed_fulllegend.png"),
    plot = p_lng_europe, width = 8.0, height = 4.6, bg = "white", dpi = 300
  )
  
  # =========================== C. 合并（各自 legend；右侧灰条；标题一致） ===========================
  p_lng_eu_strip  <- add_right_strip(p_lng_eu,  "LNG",      w_bar_data = 0.05)
  p_pipe_eu_strip <- add_right_strip(p_pipe_eu, "Pipeline", w_bar_data = 0.05)
  
  p_europe_combined <- (p_lng_eu_strip / p_pipe_eu_strip) +
    patchwork::plot_layout(heights = c(1, 1)) +
    patchwork::plot_annotation(
      title = "Traded LNG and Pipeline – Europe",
      theme = ggplot2::theme(
        plot.title = ggplot2::element_text(size = 12, face = "bold", hjust = 0)
      )
    )
  ...
  ggplot2::ggsave(
    filename = file.path(output_dir, "IAMC_EUR_traded_LNG_and_pipeline.png"),
    plot = p_europe_combined, width = 7.2, height = 6.0, dpi = 300, bg = "white"
  )
  
  
  ##￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥￥
  # ---------------------------------------------------------
  # Europe LNG: high-methane producers — multi-year (2025/2035/2060)
  # ---------------------------------------------------------
  
  # 1) 与你分组函数一致的 high-methane 列表
  other_high <- c(
    "Ukraine","South Korea","USA","South America_Southern","Indonesia",
    "Central Asia","Africa_Northern","Africa_Western","Africa_Eastern",
    "Central America and Caribbean","India","EU-15","Brazil","Africa_Southern",
    "Colombia","Argentina","Taiwan","EU-12","Europe_Non_EU"
  )
  
  # 2) 自动识别年份列（支持 "2025...1" 之类）
  years_wanted <- c("2025","2035","2060")
  
  year_cols <- names(df_all)[grepl(paste0("^(", paste(years_wanted, collapse = "|"), ")(\\.\\.\\.[0-9]+)?$"), names(df_all))]
  if (length(year_cols) == 0) stop("找不到目标年份列。")
  
  # 3) 过滤并汇总
  df_eu_lng_high_multi <- df_all %>%
    dplyr::filter(
      Variable == "traded gas by tech",
      trimws(region) %in% europe_parts,
      stringr::str_detect(technology, stringr::regex("LNG", ignore_case = TRUE)),
      Source %in% scen_levels
    ) %>%
    dplyr::mutate(
      src_base = normalize_name(technology)
    ) %>%
    dplyr::filter(!(src_base %in% europe_parts), src_base %in% other_high) %>%
    dplyr::group_by(Source, producer = src_base) %>%
    dplyr::summarise(
      across(all_of(year_cols), ~ sum(suppressWarnings(as.numeric(.x)), na.rm = TRUE)),
      .groups = "drop"
    )
  
  # 4) pivot long —> year 一列
  df_eu_lng_high_multi_long <- df_eu_lng_high_multi %>%
    tidyr::pivot_longer(
      cols = all_of(year_cols),
      names_to = "year",
      values_to = "value"
    ) %>%
    dplyr::mutate(year = gsub("\\.\\.\\..*$", "", year)) %>% # 清理 "2060...1" 类尾巴
    dplyr::arrange(Source, year, desc(value))
  
  # 5) 写出 CSV
  out_csv <- file.path(output_dir, "Europe_LNG_high_methane_producers_multiYear_by_producer.csv")
  readr::write_csv(df_eu_lng_high_multi_long, out_csv)
